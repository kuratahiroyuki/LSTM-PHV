...
def training_w2v(data_path, out_path, k_mer = 4, vector_size = 128, window_size = 3, iteration = 1000):
    seq_list = import_fasta(data_path)
    k_mers_list = mer_vec(seq_list, k_mer = k_mer)
    
    logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.INFO)
    model = word2vec.Word2Vec(k_mers_list, size = vector_size, min_count = 1, window = window_size, iter = iteration, sg = 0)
